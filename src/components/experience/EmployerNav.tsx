...
export const EmployerNav: React.FC<{ currentSlug: EmployerSlug }> = ({ currentSlug }) => {
  const prev = getPrevEmployer(currentSlug);
  const next = getNextEmployer(currentSlug);
    ...
    <nav aria-label="Navigation entre employeurs" className="pt-12 pb-6 border-t border-slate-200/80 dark:border-slate-800/80">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        >
          ...
          <span className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            {prev.period} • {prev.projects.length} projets
          </span>
        </Link>
        ...
        >
          ...
          <span className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            {next.period} • {next.projects.length} projets
          </span>
        </Link>
      ...
      <div className="mt-6 text-center">
        ...
        >
          ...
          <span>Retour à la vue d'ensemble de toutes les expériences</span>
        </Link>
      </div>
    </nav>
  );
